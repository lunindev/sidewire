    ...
    private func decodeCombinedNALs(bytes: UnsafeBufferPointer<UInt8>, ranges: [(start: Int, count: Int)], formatDescription: CMVideoFormatDescription, isKeyframe: Bool) {
        ...
        reusableBlockData.withUnsafeMutableBytes { dest in
            ...
            for r in ranges {
                ...
                memcpy(ptr.advanced(by: offset), &len, 4)
                offset += 4
                memcpy(ptr.advanced(by: offset), bytes.baseAddress!.advanced(by: r.start), r.count)
                offset += r.count
            }
        }
        ...
        VTDecompressionSessionDecodeFrame(
            ...
            var outputTiming = CMSampleTimingInfo(
                duration: CMTime(value: 1, timescale: 60),
            ...
            CMSampleBufferCreateForImageBuffer(
                ...
                sampleTiming: &outputTiming,
                sampleBufferOut: &outputSampleBuffer
            )

            if let outputSampleBuffer {
                self?.onDecodedFrame?(outputSampleBuffer)
            }
        }
    }
