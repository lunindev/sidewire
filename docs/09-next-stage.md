...
5. **PAKE for pairing (security decision — surfaced by the Phase 7 security review).** The v2 channel-bound PIN proof stops proof *relay* but not *offline PIN guessing* by an **active on-path attacker at pairing time**: that attacker can offline-crack the 6-digit PIN from the Source's first proof (~20 bits → sub-second). Off-path/passive attackers and paired reconnects are unaffected; the input-injection and key-pinning gates still hold for everyone else. The robust fix is an augmented **PAKE (SPAKE2+ / CPace)** — a real, security-critical undertaking (correct group generators, key confirmation, matched Swift **and** Rust implementations), so it is the owner's call, not an autonomous change. **Options:** (a) implement SPAKE2+ now, before the Rust client, so both sides get it once; (b) ship v2 with the documented limitation — acceptable for a trusted-LAN/Thunderbolt tool (the product's stated use), revisit if it's ever exposed to hostile networks; (c) defer to a dedicated security phase after the Rust client exists. **Recommendation: (a) if the Rust client is imminent** (do the crypto once, on both sides together); otherwise (b) with the honest docs/05 threat-model note already in place. Meanwhile the review's clean bugs are fixed: the non-P256 fail-open (auth bypass) now fails closed, and the "one side forgot the pin" deadlock re-pairs.
