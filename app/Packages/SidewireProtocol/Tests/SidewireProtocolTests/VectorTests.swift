        ...
        let doc = FrameDoc(
            ...
            vectors: [
                ...
                fv("hello_ack", "HELLO_ACK frame — same JSON shape as HELLO, only the type byte differs.",
                   ...
                   payload: Data("{\"x\":1}".utf8)),
                fv("pair_msg", "PAIR_MSG frame: 32-byte CPace share (example bytes; real shares in pairing-vectors.json).",
                   type: MessageType.pairMsg.rawValue, seq: 0,
                   payload: Data((0..<32).map { UInt8($0) })),
                fv("pair_confirm", "PAIR_CONFIRM frame: 64-byte HMAC-SHA512 key-confirmation tag (example bytes).",
                   type: MessageType.pairConfirm.rawValue, seq: 1,
                   payload: Data((0..<64).map { UInt8($0) })),
