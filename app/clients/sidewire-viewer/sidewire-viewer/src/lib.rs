...
//!
//! M2 added decode + a window; M3 added borderless fullscreen, input capture ([`input`], winit
//! events → HID `InputEventRecord`s), and the ≤2.5 s heartbeat/watchdog + re-listen liveness
//! contract in [`session`]/[`wire`]. **M4** (this milestone) adds mDNS [`discovery`] (the Display
//! advertises `_sidewire._tcp`) + a graceful window-close `BYE`, plus packaging scaffolding.
