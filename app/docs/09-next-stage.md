...
- **[not done]** CI per the docs/08 design: `.github/workflows/` does not exist yet. `ci.yml` (build + both test suites on push/PR, ad-hoc signing) and `release.yml` (signed/notarized/appcast release) are both still to be written — see [`TODO.md`](../../TODO.md).
